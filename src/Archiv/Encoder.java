...
private int[] sdr = new int[Constants.SDR_SIZE*Constants.NGRAMM_SIZE]; // Für sequenz

    public static HashMap<BigInteger, String> map = new HashMap<>();

    public static int[] encodeToSDR(String text) {
        int[] sdr = new int[Constants.SDR_SIZE * Constants.NGRAMM_SIZE]; // Leerer SDR-Vektor
        ...
        for (int i = 0; i < words.size(); i++) {
            ...
            for (String sub : substrings) {
                ...
                int hash = Math.abs(sub.hashCode() % Constants.SDR_SIZE); // Hash berechnen und modulieren
                sdr[hash +
        ...
        for (int i = 0; i < Constants.NGRAMM_SIZE; i++) {
            ...
            BigInteger hashKey = createKey(Arrays.copyOfRange(sdr, i * Constants.SDR_SIZE,
    ...
    // Hilfsfunktion, um alle möglichen Teilzeichenfolgen eines Wortes zu erhalten
